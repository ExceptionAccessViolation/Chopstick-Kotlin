class Hand (val player: Player, val direction: Direction) {
    ...
    fun add() { // same as transfer
        ...
        println("${this.player.name}'s ${this.direction.name} added its value to ${otherHand.player.name}'s " +
    ...
    private fun otherHand(): Hand { // return the other hand of the respective player
        return when (this.player) {
            Player.Player1 -> if (this.direction == Direction.left) hand2 else hand1
            Player.Player2 -> if (this.direction == Direction.left) hand4 else hand3
        }
    }
}